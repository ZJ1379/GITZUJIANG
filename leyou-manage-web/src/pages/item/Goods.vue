    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon @click="deleteSkuBySpuId(props.item.id)">
            <i class="el-icon-delete"/>
          </v-btn>
          <v-btn icon v-if="props.item.saleable" @click="upOrDownBySpuId('0',props.item.id)">下架</v-btn>
          <v-btn icon v-else @click="upOrDownBySpuId('1',props.item.id)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
  ...
  export default {
    ...
    methods: {
      ...
      async editGoods(oldGoods) {
        ...
        oldGoods.skus = await this.$http.loadData("/item/goods/list?id=" + oldGoods.id);
        // 修改标记
        this.isEdit = true;
        // 控制弹窗可见：
        this.show = true;
      ...
      deleteSkuBySpuId(id){
        ...
        this.$message.confirm('此操作将永久删除数据，是否继续?', '提示', {
          ...
          this.$http.get("/item/goods/deleteSkuBySpuId/" +id)
            .then(({data}) => {
      ...
      upOrDownBySpuId(s,id){
        ...
        this.$message.confirm('此操作将商品'+name+'，是否继续？', '提示', {
          ...
          //this.handleDelete(this.model.id);
          this.$http.get("/item/goods/upOrDownSaleableBySpuId?" ,{
            params:{
              saleable:s,
              spuId:id
            }
